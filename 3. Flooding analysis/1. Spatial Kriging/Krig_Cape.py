...
#%%-----------------CAPE FEAR------------------------------------------------##
#%%Krig norm CAPE FEAR and extent
krig_CF_min = 0
krig_CF_max = 13
...
extent_mat = (top_left_lon, top_left_lon + CapeFear_WS.shape[1] * 0.3625, top_left_lat - CapeFear_WS.shape[0] * 2.75, top_left_lat)

#%%Defining the grid
#xx, yy = np.mgrid[33.75:36.50:37j, -80.20:-77.30:-77j]

#xv = CF_HWM['Long'].values
#yv = CF_HWM['Lat'].values

xv = np.arange(-80.20, -77.30, 0.01)
yv = np.arange(33.75, 36.50, 0.01)
...
for k in range(my_start,my_end):
    k = h_snap[my_start]    
    # Variogram
    VCF = skg.Variogram(CF_HWM[['Long', 'Lat']].values, CF_HWM[str(k)].values, maxlag='median', normalize=False)
    VCF_params = VCF.describe()
    
    #Kriging
    ok = OrdinaryKriging(VCF, min_points=5, max_points=20, mode='exact')
   ...
   #plt.colorbar(ctr_hh, cax=cax, label = 'Flooding depth [ft]')
    ...
    CF_HWM.plot(ax = ax, c = "black", marker= '.', markersize = 2)
    CapeFear_WS.plot(ax = ax, color = 'bisque', edgecolor = 'dimgray',alpha = 0.4)
    ...
    for i in range(maskin.shape[0]):
        for j in range(maskin.shape[1]):
            ...
            if Point(xi, yj).within(CapeFear_WS["geometry"][0]):
                maskin[i, j] = 0
            else:
                maskin[i, j] = 1
    
    # We apply our mask to previous calculated arrays:
    depth_ma = np.ma.masked_array(field, maskin)
    ...
    # Contour fringes of the kriging process:
    ctr_hh = ax.contourf(xx, yy, depth_ma, 
                         vmin = 0,
                         vmax = 13,
                     #   range(0, 13, 1),
                         cmap = "viridis_r", 
    ...
    tif_name = Kcf+"Kriging_CF_"+str(k)+".tif"
    ...
    output_raster.GetRasterBand(1).WriteArray(np.transpose(np.flip(depth_ma[::-1]))) 
