...
#%%-----------------WHITE OAK------------------------------------------------##
#%%Krig norm WHITE OAK  and extent
krig_WO_min = 0
krig_WO_max = 8

#gridx = np.arange(-78.00, -76.25, 0.01)
#gridy = np.arange(33.80, 35.00, 0.01)

top_left_lon = -78.00
top_left_lat = 35.00

extent_mat = (top_left_lon, top_left_lon + WhiteOak_WS.shape[1] * 0.21875, top_left_lat - WhiteOak_WS.shape[0] * 1.2, top_left_lat)

#%%Defining the grid
xv = np.arange(-78.00, -76.25, 0.01)
yv = np.arange(33.80, 35.00, 0.01)
...
h_snap_WO = h_snap#h_snap.head(256)

for k in range(my_start,my_end):
    k = h_snap_WO[my_start]
    # Variogram
    V_WO = skg.Variogram(WOak_HWM[['Long', 'Lat']].values, WOak_HWM[str(k)].values, maxlag='median', normalize=False)
    V_WO_params = V_WO.describe()
    
    #Kriging
    ok = OrdinaryKriging(V_WO, min_points=5, max_points=20, mode='exact')
    
    field_WO = ok.transform(xx.flatten(), yy.flatten()).reshape(xx.shape)
    s2_WO = ok.sigma.reshape(xx.shape)
    ...
    ctr_hh = ax.contourf(xx, yy, field_WO,
                         vmin = krig_WO_min,
                         vmax = krig_WO_max,
                     #   range(0,13,1),
                         cmap = "viridis_r", 
    ...
    WOak_HWM.plot(ax = ax, c = "black", marker= '.', markersize = 2)
    WhiteOak_WS.plot(ax = ax, color = 'bisque', edgecolor = 'dimgray',alpha = 0.4)
    
    ax = axs[1]
    
    # Contour fringes of the kriging error:
    ctr_err = ax.contourf(xx, yy, s2_WO,
                          #vmin = 0,
                          #vmax = 20,
                         #range(0,20,2),
                          cmap = "plasma",
    ...
    for i in range(maskin.shape[0]):
        for j in range(maskin.shape[1]):
            ...
            if Point(xi, yj).within(WhiteOak_WS["geometry"][0]):
                maskin[i, j] = 0
            else:
                maskin[i, j] = 1
    
    # We apply our mask to previous calculated arrays:
    depth_m_WO = np.ma.masked_array(field_WO, maskin)
    s2_m_WO = np.ma.masked_array(s2_WO, maskin)
    ...
    # Contour fringes of the kriging process:
    ctr_hh = ax.contourf(xx, yy, depth_m_WO, 
                         vmin = krig_WO_min,
                         vmax = krig_WO_max,
                     #   range(0, 13, 1),
                         cmap = "viridis_r", 
    ...
    #%% Contour fringes of the kriging error:
    ctr_err = ax.contourf(xx, yy, s2_m_WO,
                          #vmin = 0,
                          #vmax = 20,
                      #   range(0,20,2),
                          cmap = "plasma",
    ...
    tif_name = Kwo+"Kriging_WO_"+str(k)+".tif"
    ...
    output_raster.GetRasterBand(1).WriteArray(np.transpose(np.flip(depth_m_WO[::-1]))) 
