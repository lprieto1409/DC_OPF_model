...
#%%-----------------YADKIN------------------------------------------------##
#%%Krig norm YADKIN PEE DEE  and extent
krig_YPD_min = 0
krig_YPD_max = 9
...
extent_mat = (top_left_lon, top_left_lon + YadkinPeeDee_WS.shape[1] * 0.3125, top_left_lat - YadkinPeeDee_WS.shape[0] * 2, top_left_lat)

#%%Defining the grid
xv = np.arange(-82.00, -79.50, 0.01)
yv = np.arange(34.75, 36.75, 0.01)
...
h_snap_YPD = h_snap.head(256)

for k in range(my_start,my_end):
    k = h_snap_YPD[my_start]
    # Variogram
    V_YPD = skg.Variogram(Yadkin_HWM[['Long', 'Lat']].values, Yadkin_HWM[str(k)].values, maxlag='median', normalize=False)
    V_YPD_params = V_YPD.describe()
    
    #Kriging
    ok = OrdinaryKriging(V_YPD, min_points=5, max_points=20, mode='exact')
    
    field_YPD = ok.transform(xx.flatten(), yy.flatten()).reshape(xx.shape)
    s2_YPD = ok.sigma.reshape(xx.shape)
    ...
    ctr_hh = ax.contourf(xx, yy, field_YPD,
                         vmin = krig_YPD_min,
                         vmax = krig_YPD_max,
                     #   range(0,13,1),
                         cmap = "viridis_r", 
    ...
    Yadkin_HWM.plot(ax = ax, c = "black", marker= '.', markersize = 2)
    YadkinPeeDee_WS.plot(ax = ax, color = 'bisque', edgecolor = 'dimgray',alpha = 0.4)
    
    ax = axs[1]
    
    # Contour fringes of the kriging error:
    ctr_err = ax.contourf(xx, yy, s2_YPD,
                          #vmin = 0,
                          #vmax = 20,
                         #range(0,20,2),
                          cmap = "plasma",
    ...
    for i in range(maskin.shape[0]):
        for j in range(maskin.shape[1]):
            ...
            if Point(xi, yj).within(YadkinPeeDee_WS["geometry"][0]):
                maskin[i, j] = 0
            else:
                maskin[i, j] = 1
    
    # We apply our mask to previous calculated arrays:
    depth_m_YPD = np.ma.masked_array(field_YPD, maskin)
    s2_m_YPD = np.ma.masked_array(s2_YPD, maskin)
    ...
    # Contour fringes of the kriging process:
    ctr_hh = ax.contourf(xx, yy, depth_m_YPD, 
                         vmin = krig_YPD_min,
                         vmax = krig_YPD_max,
                     #   range(0, 13, 1),
                         cmap = "viridis_r", 
    ...
    #%% Contour fringes of the kriging error:
    ctr_err = ax.contourf(xx, yy, s2_m_YPD,
                          #vmin = 0,
                          #vmax = 20,
                      #   range(0,20,2),
                          cmap = "plasma",
    ...
    tif_name = Kypd+"Kriging_YPD_"+str(k)+".tif"
    ...
    output_raster.GetRasterBand(1).WriteArray(np.transpose(np.flip(depth_m_YPD[::-1]))) 
