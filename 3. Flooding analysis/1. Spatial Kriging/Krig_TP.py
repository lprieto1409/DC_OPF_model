...
#%%-----------------TAR PAMlico------------------------------------------------##
#%%Krig norm TAR PAMLICO  and extent
krig_TP_min = 0
krig_TP_max = 5

#gridx = np.arange(-79.00, -75.00, 0.01)
#gridy = np.arange(34.50, 36.50, 0.01)

top_left_lon = -79.00
top_left_lat = 36.50

##FALTA#
extent_mat = (top_left_lon, top_left_lon + TarPamlico_WS.shape[1] * 0.5, top_left_lat - TarPamlico_WS.shape[0] * 2.0, top_left_lat)

#%%Defining the grid
xv = np.arange(-79.00, -75.00, 0.01)
yv = np.arange(34.50, 36.50, 0.01)
...
h_snap_TP = h_snap#h_snap.head(256)

for k in range(my_start,my_end):
    k = h_snap_TP[my_start]
    # Variogram
    V_TP = skg.Variogram(TarPamli_HWM[['Long', 'Lat']].values, TarPamli_HWM[str(k)].values, maxlag='median', normalize=False)
    V_TP_params = V_TP.describe()
    
    #Kriging
    ok = OrdinaryKriging(V_TP, min_points=5, max_points=20, mode='exact')
    
    field_TP = ok.transform(xx.flatten(), yy.flatten()).reshape(xx.shape)
    s2_TP = ok.sigma.reshape(xx.shape)
    ...
    ctr_hh = ax.contourf(xx, yy, field_TP,
                         vmin = krig_TP_min,
                         vmax = krig_TP_max,
                     #   range(0,13,1),
                         cmap = "viridis_r", 
    ...
    TarPamli_HWM.plot(ax = ax, c = "black", marker= '.', markersize = 2)
    TarPamlico_WS.plot(ax = ax, color = 'bisque', edgecolor = 'dimgray',alpha = 0.4)
    
    ax = axs[1]
    
    # Contour fringes of the kriging error:
    ctr_err = ax.contourf(xx, yy, s2_TP,
                          #vmin = 0,
                          #vmax = 20,
                         #range(0,20,2),
                          cmap = "plasma",
    ...
    for i in range(maskin.shape[0]):
        for j in range(maskin.shape[1]):
            ...
            if Point(xi, yj).within(TarPamlico_WS["geometry"][0]):
                maskin[i, j] = 0
            else:
                maskin[i, j] = 1
    
    # We apply our mask to previous calculated arrays:
    depth_m_TP = np.ma.masked_array(field_TP, maskin)
    s2_m_TP = np.ma.masked_array(s2_TP, maskin)
    ...
    # Contour fringes of the kriging process:
    ctr_hh = ax.contourf(xx, yy, depth_m_TP, 
                         vmin = krig_TP_min,
                         vmax = krig_TP_max,
                     #   range(0, 13, 1),
                         cmap = "viridis_r", 
    ...
    #%% Contour fringes of the kriging error:
    ctr_err = ax.contourf(xx, yy, s2_m_TP,
                          #vmin = 0,
                          #vmax = 20,
                      #   range(0,20,2),
                          cmap = "plasma",
    ...
    tif_name = Ktp+"Kriging_TP_"+str(k)+".tif"
    ...
    output_raster.GetRasterBand(1).WriteArray(np.transpose(np.flip(depth_m_TP[::-1]))) 
