...
#%%-----------------LUMBER------------------------------------------------##
#%%Krig norm LUMBER  and extent
krig_LU_min = 0
krig_LU_max = 6

#gridx = np.arange(-80.00, -78.00, 0.01)
...
##FALTA#
extent_mat = (top_left_lon, top_left_lon + Lumber_WS.shape[1] * 0.25, top_left_lat - Lumber_WS.shape[0] * 1.6, top_left_lat)

#%%Defining the grid
xv = np.arange(-80.00, -78.00, 0.01)
yv = np.arange(33.80, 35.40, 0.01)
...
h_snap_LU = h_snap#h_snap.head(256)

for k in range(my_start,my_end):
    k = h_snap_LU[my_start]
    
    # Variogram
    V_LU = skg.Variogram(Lumb_HWM[['Long', 'Lat']].values, Lumb_HWM[str(k)].values, maxlag='median', normalize=False)
    V_LU_params = V_LU.describe()
    
    #Kriging
    ok = OrdinaryKriging(V_LU, min_points=5, max_points=20, mode='exact')
    
    field_LU = ok.transform(xx.flatten(), yy.flatten()).reshape(xx.shape)
    s2_LU = ok.sigma.reshape(xx.shape)
    ...
    ctr_hh = ax.contourf(xx, yy, field_LU,
                         vmin = krig_LU_min,
                         vmax = krig_LU_max,
                     #   range(0,13,1),
                         cmap = "viridis_r", 
    ...
    Lumber_WS.plot(ax = ax, color = 'bisque', edgecolor = 'dimgray',alpha = 0.4)
    
    ax = axs[1]
    
    # Contour fringes of the kriging error:
    ctr_err = ax.contourf(xx, yy, s2_LU,
                          #vmin = 0,
                          #vmax = 20,
                         #range(0,20,2),
                          cmap = "plasma",
    ...
    depth_m_LU = np.ma.masked_array(field_LU, maskin)
    s2_m_LU = np.ma.masked_array(s2_LU, maskin)
    ...
    # Contour fringes of the kriging process:
    ctr_hh = ax.contourf(xx, yy, depth_m_LU, 
                         vmin = krig_LU_min,
                         vmax = krig_LU_max,
                     #   range(0, 13, 1),
                         cmap = "viridis_r", 
    ...
    #%% Contour fringes of the kriging error:
    ctr_err = ax.contourf(xx, yy, s2_m_LU,
                          #vmin = 0,
                          #vmax = 20,
                      #   range(0,20,2),
                          cmap = "plasma",
    ...
    tif_name = KLumber+"Kriging_LU_"+str(k)+".tif"
    ...
    output_raster.GetRasterBand(1).WriteArray(np.transpose(np.flip(depth_m_LU[::-1]))) 
